...
/** @typedef {'concept'|'kit'|'reference'|'ui'|'cartography'|'font'|'icon'|'branding'} Category */
/** @typedef {'act0'|'act1'|'act2'|'act3'|'kits'|'none'} Act */
/** @typedef {'ld'|'scene'|'char'|'kit'|'legacy'|'menu'|'hud'|'dialogue'|'quest'|'font'|'map'|'icon'|'other'} Layer */
 ...
 * @property {string} kind  image | font
...
async function main() {
  ...
  for (const c of candidates) {
    ...
    const kind = FONT_EXT.has(ext) ? 'font' : 'image'
    ...
    if (kind === 'image') {
      ...
      const made = await ensureThumb(c.abs, thumbAbs)
      if (made) {
