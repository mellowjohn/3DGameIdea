...
Password-gated React **Art Atlas** for collaborators: overview landing, browsable catalog (including runtime glTF orbit review), comments with likes/dislikes.
...
Concepts, kits, reference, branding, curated cartography, UI concepts, fonts, item icons, and curated runtime **glTF models** from `samples/open-world-rpg/assets/models/` (plus sibling textures). Stub/clip-only files (`player_clips`, `dead-tree`) are skipped. Tile pyramids stay excluded.

Browse cards lazy-load a small auto-rotating mesh preview (`@google/model-viewer`); open the lightbox for full orbit controls. Atlas PNGs are not used as card posters. Comments work the same as image assets.
...
React 19 + Vite 8 + react-router-dom, `@google/model-viewer`, sharp thumbs, Vercel Edge Middleware, Upstash Redis (optional) for comments.
