...
- **Modular clothing from a player copy:** retain the player's exact skeleton, bind pose, and animation set. Keep each complete authored mesh; correct fit by adjusting its shell positions in Blockbench against the base body, rather than deleting faces at export. Combined source: `tools/art/modular-iron-test/GoodPlayerModelCopy.bbmodel` with catalog `generic.keepMeshes` (helmet / torso set / legs). Never bake that file as `player`. Set `generic.matchPlayerBake: true` so the kit inherits `player.gltf`'s height-normalize scale and Hips offset — otherwise GPU skinning applies player palettes (2.75 m bind) to raw Blockbench verts and the shell floats off the body. Duplicate mesh names keep the densest copy. Every modular target must set `requireDedicatedAtlas: true` and choose `dedicated_embedded` (painted atlas) or `dedicated_material` plus `flatAtlasRgb` (temporary single-colour material). Re-bake with `engine asset-bake --target <kit-id>` and verify head/chest/legs in Animation → Armor slots. Runtime uploads a per-piece bone palette from the player's animated locals × the shell's own inverse binds. Do not use instance `armorAttach` scale — it is origin-centered and will not track the head.
...
`tessera_banner_pole.gltf` / `corner_tower_stub.gltf` / `weapon_rack.gltf` / `workbench_table.gltf` are the A0-02 appearance-courtyard kit bakes (Blockbench free-mesh). Sources under `tools/art/{tessera-banner-pole,corner-tower-stub,weapon-rack,workbench-table}/`. Rebake with `python tools/bake_appearance_courtyard_kit.py` (optional name filter) or `engine asset-bake --target tessera_banner_pole` (and siblings). Heights ≈ 3.4 m / 5.4 m / 2.15 m / 1.05 m; banner and rack emit double-sided tris for thin cloth/bow faces. Prefabs: Scene Assets `tessera_banner_pole`, `corner_tower_stub`, `weapon_rack`, `workbench_table` (staticWorld capsule/box). Concept: `context/art/act0-appearance-courtyard-kit-concept.png`.

`test_hair_spikes.gltf` is the appearance-socket **short hair** test (Head-weighted cap on GoodPlayerModel). Source mesh `kit_test_hair_spikes` in `tools/art/modular-iron-test/GoodPlayerModelCopy.bbmodel`. Rebake with `engine asset-bake --target test_hair_spikes` (`matchPlayerBake`, dedicated brown atlas). Not inventory `head` armor — `.character.json` `appearance.hairMesh` + Animation Studio appearance sockets.
...
`target_dummy.gltf` is the combat training dummy (Blockbench free-mesh, **skinned**). Source: `tools/art/target-dummy/Target_Dummy.bbmodel`. Rebake with `engine asset-bake --target target_dummy` or `python tools/bake_target_dummy.py` (feet at y=0, height ≈ 2.2 m, atlas `target_dummy.png`, clips `Idle` + `HitReact`). Prefab: `assets/prefabs/Scene Assets/target_dummy.prefab.json` (staticWorld capsule + `combatHurt: dummy_body` + animator `assets/animators/target_dummy.animator.json`). Concept: `context/art/reference/target-dummy-turnaround.png`. Playable pad: `worlds/combat-sandbox.world.json`. Hits fire Lua `hit` → dummy HitReact, then return to Idle.
