...
Enum `AuthoredComponentType`: `Collider`, `ScriptBinding`, `Animator`, `Rigidbody`, `AudioSource`. Stored on the entity as `AuthoredComponentsComponent` (`entries[]` + `generation`).

JSON type strings: `"collider"`, `"scriptBinding"`, `"animator"`, `"rigidbody"`, `"audioSource"`.
...
**AudioSource (TICKET-0210 / 0211):** Unity-like clip source on entities/prefabs. Fields: `clip` (project-relative `.wav`), `volume` (0–1), `loop`, `spatial`, `playOnStart`, `minDistance`, `maxDistance`. Schema + Inspector/Prefab Editor Add Component shipped in 0210; runtime `playOnStart` and Lua `play_entity_audio` are TICKET-0211. Session `AudioEngine` + Lua `play_sound*` remain available (TICKET-0107).
...
| AudioSource (authored) | schema/inspector (0210); runtime playOnStart + Lua trigger (0211) | [`features/audio.md`](../features/audio.md) |
...
| Item | Status |
| --- | --- |
| New authored kinds beyond collider / scriptBinding / animator / rigidbody / audioSource | Out of scope — do not invent here |
