...
Enum `AuthoredComponentType`: `Collider`, `ScriptBinding`, `Animator`. Stored on the entity as `AuthoredComponentsComponent` (`entries[]` + `generation`).

JSON type strings: `"collider"`, `"scriptBinding"`, `"animator"`.
...
| Entity data | Same shape fields as prefab volumes: `shape`, `layer`, `trigger`, optional `interaction` / `combatHit` / `combatHurt`, local `transform`, `halfExtent` or `radius` |
...
| Character / root motion | Animator + character controller (not a separate authored component type) | [`character-controller.md`](../features/character-controller.md) |
...
| Item | Status |
| --- | --- |
| New authored kinds beyond collider / scriptBinding / animator | Out of scope — do not invent here |
