...
The product is a Windows-first C++20 and Direct3D 12 engine for a single-player third-person action RPG in a seamless **16×16 km** world ([DEC-0054](../decisions/index.md#dec-0054-continent-scale-seamless-world--stream-budget)). The runtime uses a hybrid ECS and authored hierarchy, with Lua for content-heavy gameplay logic.
...
- Gameplay luck (loot, crits, procs, table picks) uses named **marble bags** — integer counts, draw without replacement, persist remainder — not independent `random < p` rolls ([DEC-0057](../decisions/index.md#dec-0057-marble-bag-rng-for-gameplay-rolls)). Particle/foliage noise stays independent.
...
- The **16×16 km** world is partitioned into 128 m cells. Cell addressing uses mathematical floor semantics so negative coordinates map correctly.
