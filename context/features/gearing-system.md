...
**Runtime (2026-07-30, TICKET-0237 + follow-on):** `InventoryRuntime` + `ItemCatalogAsset` load `assets/items/*.json`. Lua `engine.inventory_*` (incl. `inventory_move`) + MCP `engine_inventory_call` (`move` kind). Play-test grants Ashfell arming sword to hotbar 0 + field bandages; loot bag/chest call `inventory_grant`. Inventory modal aligned to `inventory-ui.pen` (equip + 4 trinkets + bag grid + footer hotbar 8; bag-upgrade stubs). **Drag-and-drop** between bag / hotbar / equip slots (select click still works).
...
**Play-test Inspector → Held Weapon Weld** picks the joint from a dropdown of the **live skin's joints** (all 37 on the player rig, not a fixed shortlist), edits grip offset / Euler / scale, enables a **Move / Rotate / Scale gizmo** with **Local/World** space and optional **snapping** (Scene or Game view; **G**/**R**/**T**/**X** — not 1–2), and **Save handAttach** writes `joint`, `gripOffset`, `gripEulerDeg`, `gripScale` into the item's source JSON. The socket **freezes for the duration of a drag** so an animating joint cannot pull the handles out from under the cursor; Pause (F6) still freezes body and weld together for a fully static pose.

Default joint for an item with no authored `handAttach` is `RightHand`. Welds read the same joint globals the skin does, so they inherit the sagittal clip mirror from the RH→LH import (see [`../testing/findings.md`](../testing/findings.md)): while a clip plays, `RightHand` is the on-screen right hand, but an un-animated bind pose puts that joint on the opposite side. Author welds during play-test, not against the bind pose.
