...
/// Write tightly packed 8-bit RGBA pixels to a PNG under `project_root/out/` (timestamped stem).
...
/// Write tightly packed 8-bit RGBA pixels to an exact PNG path (creates parent dirs).
[[nodiscard]] Result<std::filesystem::path> write_rgba_png_path(const std::filesystem::path& out_path,
    std::uint32_t width, std::uint32_t height, std::span<const std::uint8_t> rgba_bytes);
